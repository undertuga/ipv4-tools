...
IPv4Tools.prototype.getNetworkClass = function(ipv4, callback){
	...
	if(
		...
		if(ipv4 !== null){callback(null, false); return;} // fail safe bail out
	}
};
	    ...
	    async.waterfall([
          ...
          */
	        ...
	        function(asn, callback){
	            ...
	            // validating result
	            if(!results){callback(null, false); return;}
	            else{callback(null, buffer); return;}
	        }
	    );
	}
};
	...
	if(((typeof(ipv4) === 'undefined') || (ipv4 === null) || (ipv4 === "") || (ipv4.length <= 0) || (ipv4.length > 16))){callback(null, false);}
		...
		dns.reverse(ipv4, function(error, dnsrev){
			...
			if(error){callback(null, false); return;}
				...
				if(dnsrev.length <= 0){callback(null, false); return;}
				else{
					...
					async.eachSeries(dnsrev, function(domain, callback){
						...
						if(error){callback(null, false); return;}
						else{callback(null, dnsdata); return;}
					});
				}
			}
		});
